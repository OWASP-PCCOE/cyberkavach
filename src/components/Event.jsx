  ...
  return (
    ...
    >
      ...
      <div className="flex-grow flex flex-col items-center justify-evenly  text-white h-auto relative -translate-y-[40px] md:-translate-y-[5px]">
        ...
        {/* Conditionally render the Register Button */}
        {eventId !== 8 && eventId !== 9 && eventId !== 1 && (
          <div className="mt-4 rounded-xl" style={{ opacity: contentOpacity, zIndex: 15 }}>
            <button
              onClick={() => gForm(gFormLink)}
              style={{ fontFamily: 'Gilroy' }}
              className="p-3 rounded-xl pl-20 pr-20 border-2 neon-slate border-white text-white font-semibold md:text-[25px]"
        ...
        <div
          id="slider"
          ref={contentRef}
          className={`absolute bottom-[222px] md:bottom-[50px] transition-all duration-500 ease-out bg-gray-800 rounded-lg text-center p-4 z-10 ${isVisible ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 translate-y-full pointer-events-none'}`}
          style={{
            width: 'calc(100% - 40px)',
            maxWidth: '400px',
        ...
        <div id="event-up" className="w-full flex justify-center mt-2 mb-8 z-20 md:mb-0">
          <button
            ...
            onClick={handleToggle}
            className="p-2 pl-20 pr-20 rounded-[10px] text-[15px] border-2 border-rose-50 border-opacity-20"
            style={{ fontFamily: 'Gilroy', backgroundColor: '#222222', opacity: 1 }}
          >
            {isVisible ? (
